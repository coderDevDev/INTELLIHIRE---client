// PDS Scoring System Types

export interface ScoringCriteria {
  education: ScoringField;
  experience: ScoringField;
  training: ScoringField;
  eligibility: ScoringField;
  skills: ScoringField;
  awards: ScoringField;
  relevantExperience: ScoringField;
  certifications: ScoringField;
}

export interface ScoringField {
  label: string;
  maxPoints: number;
  weight: number; // Percentage weight in total (should sum to 100)
  enabled: boolean;
  description: string;
...
export const DEFAULT_SCORING_CONFIG: ScoringCriteria = {
  education: {
    label: 'Education',
    maxPoints: 20,
    weight: 20,
...
export interface PDSScoreBreakdown {
  totalScore: number;
  maxPossibleScore: number;
  percentage: number;
  criteriaScores: {
    education: CriteriaScore;
    experience: CriteriaScore;
    training: CriteriaScore;
    eligibility: CriteriaScore;
    skills: CriteriaScore;
    awards: CriteriaScore;
    relevantExperience: CriteriaScore;
...
// For Job Post Scoring Override
export interface JobScoringConfig {
  useCompanyDefault: boolean;
  useSystemDefault: boolean;
  customScoring?: Partial<ScoringCriteria>;
}

// Calculate total max score based on enabled criteria
export function calculateMaxScore(config: ScoringCriteria): number {
...
export function validateScoringWeights(config: ScoringCriteria): boolean {
