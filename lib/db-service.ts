...
export type Application = {
  ...
  status: "applied" | "screening" | "interview" | "offered" | "hired" | "rejected" | "withdrawn"
  notes?: string
  matchScore?: number
  matchDetails?: {
...
class DatabaseService {
  ...
  async getFeaturedJobs(limit = 6) {
    ...
    }).sort({ postedDate: -1 }).limit(limit).toArray();
  }
  ...
  async getGovernmentJobs(limit = 4) {
    ...
    const govCompanies = await db.collection("companies").find({ 
      isGovernment: true 
